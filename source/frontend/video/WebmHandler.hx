...
class WebmHandler
{
	...
	public function makePlayer():Void
	{
		...
		webm = new WebmPlayer();
		webm.fuck(io, false);
		webm.addEventListener(WebmEvent.PLAY, (e) -> {
			onPlay();
		});
		webm.addEventListener(WebmEvent.COMPLETE, (e) -> {
			onEnd();
		});
		webm.addEventListener(WebmEvent.STOP, (e) -> {
			onStop();
		});
		webm.addEventListener(WebmEvent.RESTART, (e) -> {
			onRestart();
		});
		webm.visible = false;
		initialized = true;
