...
/**
 * modded for VisionSphere
 */
class FlxSprite extends FlxObject
{
	public static var defaultAntialiasing:Bool = SaveData.antialiasing;
	...
	/**
	 * Whether the offset is used for rotated matrix calculations rather than the `rotationOffset`.
	 * 
	 * Defaults to false.
	 */
	public var useOffsetForRotation:Bool = false;

	/**
	 * Rotation offset point used for rotated matrix calculations when `useOffsetForRotation` is false.
	 */
	public var rotationOffset(default, null):FlxPoint;

	/**
	 * Offset angle for rotated matrix calculations.
	 */
	public var rotationOffsetAngle:Null<Float>;
	...
	override public function destroy():Void
	{
		...
		rotationOffset = FlxDestroyUtil.put(rotationOffset);
	...
	override public function draw():Void
	{
		...
		{
			if (camera == null || !camera.visible || !camera.exists || (!__firstDraw && !isOnScreen(camera)))
				continue;

			getScreenPosition(_point, camera);
			if (!useOffsetForRotation)
	...
	function drawComplex(camera:FlxCamera):Void
	{
		...
		if (rotationOffsetAngle != null && rotationOffsetAngle != angle)
		{
			var angleOff = (-angle + rotationOffsetAngle) * FlxAngle.TO_RAD;
			_matrix.rotate(-angleOff);
			if (useOffsetForRotation)
				_matrix.translate(-offset.x, -offset.y);
			else
				_matrix.translate(-rotationOffset.x, -rotationOffset.y);
			_matrix.rotate(angleOff);
		}
		else
		{
	...
	public function getScreenBounds(?newRect:FlxRect, ?camera:FlxCamera):FlxRect
	{
		...
		var offset = (useOffsetForRotation ? FlxPoint.get(0, 0) : offset);
		var rotationOffset = (useOffsetForRotation ? offset : rotationOffset);
		
		newRect.setPosition(x, y);
		if (pixelPerfectPosition)
			newRect.floor();
		var scaledOrigin = FlxPoint.weak(origin.x * Math.abs(scale.x), origin.y * Math.abs(scale.y));
		var scaledRotationOffset = FlxPoint.weak(rotationOffset.x * Math.abs(scale.x), rotationOffset.y * Math.abs(scale.y));
		...
		if (useOffsetForRotation)
			offset.put();
		return newRect.getRotatedBounds(angle, scaledOrigin, newRect, scaledRotationOffset);
	}
	...
	override function set_angle(Value:Float):Float
	{
		...
		{
			_angleChanged = true;
			if (animation != null) animation.update(0);
		}
		return ret;
	}
